...
class MAcrossover(bt.Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.fast_sma[0] > self.slow_sma[0] and self.fast_sma[-1] < self.slow_sma[-1] and self.crossover > 0:
                self.log(f'BUY CREATE {self.dataclose[0]:2f}')
                # Keep track of the created order to avoid a 2nd order
                self.order = self.buy()
            # Otherwise if the 20 SMA is below the 50 SMA
            elif self.fast_sma[0] < self.slow_sma[0] and self.fast_sma[-1] > self.slow_sma[-1] and self.crossover < 0:
...
class Screener_SMA(bt.Analyzer):
    ...
    def start(self):
        self.bbands = {data: bt.indicators.BollingerBands(
            data, period=self.params.period, devfactor=self.params.devfactor) for data in self.datas}
